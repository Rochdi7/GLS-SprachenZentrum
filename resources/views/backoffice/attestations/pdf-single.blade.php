@php
    ...
    $T = [
        'de' => [
            ...
            'participation' =>
                'hat in der Zeit vom :start bis :end an einem Deutschkurs im GLS Sprachenzentrum teilgenommen.',
            ...
            'legal' =>
                'Die Bewertung der im Kurs erzielten Ergebnisse erfolgt durch die Lehrkräfte. Diese Teilnahmebestätigung ist kein Diplom. Die Bewertungsskala umfasst 4 Stufen: sehr gut, gut, befriedigend, regelmäßige Teilnahme.',
            'place_label' => 'Ort',
            'date_label' => 'Datum',
        ...
        'en' => [
            ...
            'legal' =>
                'Course results are assessed by the teachers. This attendance certificate is not a diploma. The grading scale comprises 4 grades: very good, good, satisfactory, regular attendance.',
            'place_label' => 'Place',
            'date_label' => 'Date',
    ...
    $kursinfoLine = strtr($t['kursinfo_line'], [
        ':idx' => $attestation->stufe_index,
        ':total' => $attestation->stufe_total,
    ]);
...
<body>
    ...
    <div class="name-block">
        <div class="name-value">{{ strtoupper($attestation->last_name) }} {{ strtoupper($attestation->first_name) }}
        </div>
    ...
    <div class="check-row">
        <span
            class="check-box {{ $attestation->fees_status === 'full' ? 'checked' : '' }}"></span>{{ $t['fees_full'] }}
    </div>
    <div class="check-row">
        <span
            class="check-box {{ $attestation->fees_status === 'partial' ? 'checked' : '' }}"></span>{{ $t['fees_partial'] }}
    </div>
    ...
    <div class="erfolg-line">
        @foreach ($erfolgList as $i => $opt)
            <span>{{ $erfolgT[$opt] ?? $opt }}</span>
            @if ($i < count($erfolgList) - 1)
                ,
            @endif
        @endforeach
        .
