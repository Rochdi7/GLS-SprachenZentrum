        ...
        <form method="GET" class="card mb-3" id="geFilterForm">
            <div class="card-body py-3">
                <div class="d-flex align-items-center mb-2">
                    ...
                    @if (!$isAll)
                        <span class="badge bg-light-primary text-primary ms-2">{{ $selectedCount }} / {{ $totalAvailable }}
                            groupe(s)</span>
                    @endif
                </div>
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <input type="hidden" name="classIds" id="geClassIdsInput"
                            value="{{ implode(',', $selectedClassIds ?? []) }}">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-2 px-1">
                                    ...
                                    <div class="btn-group btn-group-sm" role="group">
                                        <button type="button" class="btn btn-link btn-sm p-0 me-2"
                                            id="geSelectAll">Tous</button>
                                        <button type="button" class="btn btn-link btn-sm p-0 text-danger"
                                            id="geSelectNone">Aucun</button>
                                    </div>
                                </div>
                                <div class="mb-2 px-1">
                                    <input type="text" class="form-control form-control-sm" id="geGroupFilter"
                                        placeholder="Rechercher…" autocomplete="off">
                                </div>
                                <div id="geGroupList">
                                    @foreach ($allGroups as $g)
                                        <label
                                            class="dropdown-item d-flex align-items-center gap-2 small py-1 ge-group-item"
                                            style="cursor:pointer;">
                                            <input type="checkbox" class="form-check-input ge-group-cb"
                                                value="{{ $g['class_id'] }}"
                    ...
                    <div class="col-auto">
                        <a href="{{ url()->current() }}?refresh=1&startDate={{ $startDate }}&endDate={{ $endDate }}{{ !empty($selectedClassIds) ? '&classIds=' . implode(',', $selectedClassIds) : '' }}"
                            class="btn btn-sm btn-outline-secondary" title="Recalculer (ignorer le cache)">
        ...
        @if (empty($groups))
            <div class="alert alert-info border-0 shadow-sm">
                ...
                <span class="small text-muted d-block mt-1">
                    (Les groupes ne sont pas filtrés par la plage de dates — la plage ne contrôle que les paiements analysés
                    pour Début / Ajouts / Quittant / Changement.)
                </span>
            </div>
        @else
            <div class="row g-3">
                    ...
                    <div class="card h-100 shadow-sm">
                        <div class="card-body">
                            ...
                            <div class="mb-2 d-flex flex-wrap gap-3 justify-content-center">
                                <span class="ge-legend"><span class="ge-dot" style="background:#6f42c1;"></span> Début
                                    (inscrits au démarrage)</span>
                                <span class="ge-legend"><span class="ge-dot" style="background:#28a745;"></span> Ajouts (Les
                                    ajouts)</span>
                                <span class="ge-legend"><span class="ge-dot" style="background:#dc3545;"></span> Départs
                                    définitifs (Quittant)</span>
                                <span class="ge-legend"><span class="ge-dot" style="background:#fd7e14;"></span> Changements
                                    de groupe</span>
                                <span class="ge-legend"><span class="ge-dot" style="background:#2196f3;"></span> Actifs
                                    actuellement</span>
                            </div>
                                    ...
                                    <tbody>
                                        @foreach ($groups as $i => $g)
                                            <tr>
                                                ...
                                                <td>{{ $g['name'] }}</td>
                                                <td class="text-center ge-cell-debut cursor-pointer"
                                                    data-group-index="{{ $i }}" data-category="debuts">
                                                    {{ $g['debuts'] ?? 0 }}
                                                </td>
                                                <td class="text-center ge-cell-ajout cursor-pointer"
                                                    data-group-index="{{ $i }}" data-category="ajouts">
                                                    {{ $g['ajouts'] }}
                                                </td>
                                                <td class="text-center ge-cell-quittant cursor-pointer"
                                                    data-group-index="{{ $i }}" data-category="quittants">
                                                    {{ $g['quittants'] }}
                                                </td>
                                                <td class="text-center ge-cell-changement cursor-pointer"
                                                    data-group-index="{{ $i }}" data-category="changements">
                                                    {{ $g['changements'] }}
                                                </td>
                                                <td class="text-center ge-cell-actif cursor-pointer"
                                                    data-group-index="{{ $i }}" data-category="actifs">
                                                    {{ $g['actifs'] }}
                                                </td>
                                            </tr>
                                        @endforeach
            ...
            <div class="row g-3 mt-1">
                <div class="col-6 col-md-4 col-xl">
                    <div class="card shadow-sm ge-kpi-card h-100">
                        <div class="card-body text-center">
                            <span class="ge-icon" style="background:#efe5fb; color:#6f42c1;"><i
                                    class="ti ti-flag-3"></i></span>
                ...
                <div class="col-6 col-md-4 col-xl">
                    <div class="card shadow-sm ge-kpi-card h-100">
                        <div class="card-body text-center">
                            <span class="ge-icon" style="background:#e3f2fd; color:#2196f3;"><i
                ...
                <div class="col-6 col-md-4 col-xl">
                    <div class="card shadow-sm ge-kpi-card h-100">
                        <div class="card-body text-center">
                            <span class="ge-icon" style="background:#e8f5e9; color:#28a745;"><i
                ...
                <div class="col-6 col-md-4 col-xl">
                    <div class="card shadow-sm ge-kpi-card h-100">
                        <div class="card-body text-center">
                            <span class="ge-icon" style="background:#fdecea; color:#dc3545;"><i
                                    class="ti ti-logout"></i></span>
                            <div class="small text-danger fw-semibold mt-2">Total des départs définitifs<br>(Quittant)
                            </div>
                ...
                <div class="col-6 col-md-4 col-xl">
                    <div class="card shadow-sm ge-kpi-card h-100">
                        <div class="card-body text-center">
                            <span class="ge-icon" style="background:#fff4e5; color:#fd7e14;"><i
                                    class="ti ti-arrows-exchange"></i></span>
                            <div class="small fw-semibold mt-2" style="color:#fd7e14;">Total des changements<br>de groupe
                                (Changement)</div>
                ...
                <div class="col-6 col-md-4 col-xl">
                    <div class="card shadow-sm ge-kpi-card h-100">
                        <div class="card-body text-center">
                            <span class="ge-icon" style="background:#f3e5f5; color:#7b1fa2;"><i
            ...
            @if (request()->boolean('debug') && !empty($report['diag']))
                ...
                <div class="card mt-3 border-warning shadow-sm">
                    <div class="card-body small">
                        ...
                        <div class="row g-2">
                            <div class="col-6 col-md-3"><strong>Classes récupérées :</strong>
                                {{ $diag['classes_fetched'] }}</div>
                            <div class="col-6 col-md-3"><strong>Classes avec START_DATE :</strong>
                                {{ $diag['classes_with_start'] }}</div>
                            <div class="col-6 col-md-3"><strong>Allocations récupérées :</strong>
                                {{ $diag['allocations_fetched'] }}</div>
                            <div class="col-6 col-md-3"><strong>Allocations avec CLASS_ID :</strong>
                                {{ $diag['allocations_with_class'] }}</div>
                            <div class="col-6 col-md-3"><strong>Couples (étudiant, classe) :</strong>
                                {{ $diag['student_class_pairs'] }}</div>
                            <div class="col-6 col-md-3"><strong>CLASS_IDs distincts :</strong>
                                {{ count($diag['distinct_class_ids']) }}</div>
                            <div class="col-6 col-md-3"><strong>Classes avec END_DATE :</strong>
                                {{ $diag['classes_with_end'] ?? 0 }}</div>
                            <div class="col-6 col-md-3"><strong>Bucket Début (étudiants) :</strong>
                                {{ $diag['total_debuts_students'] ?? 0 }} / {{ $diag['total_debuts_keyed'] ?? 0 }} classes
                            </div>
                            <div class="col-6 col-md-3"><strong>Bucket Ajout (étudiants) :</strong>
                                {{ $diag['total_ajouts_students'] ?? 0 }} / {{ $diag['total_ajouts_keyed'] ?? 0 }} classes
                            </div>
                        </div>
                        ...
                        @endif
                        @if (!empty($diag['group_details']))
                            <div class="mt-2">
                                <strong>Détails des groupes :</strong>
                                <div class="mt-1" style="max-height: 400px; overflow-y: auto;">
                                    <pre class="small bg-light p-2 rounded mb-0">{{ json_encode($diag['group_details'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                                </div>
                            </div>
                        @endif
                        @if (!empty($diag['class_inferred_starts']))
                            <div class="mt-2">
                                <strong>Class inferred start months :</strong>
                                <pre class="small bg-light p-2 rounded mb-0">{{ json_encode($diag['class_inferred_starts'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                            </div>
                        @endif
                        @if (!empty($diag['class_all_students']))
                            <div class="mt-2">
                                <strong>Class all students :</strong>
                                <pre class="small bg-light p-2 rounded mb-0">{{ json_encode($diag['class_all_students'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                            </div>
                        @endif
                        ...
                        <div class="mt-2 text-muted">
                            ...
                            Si <em>Allocations récupérées</em> = 0, l'API ne renvoie rien pour la plage choisie (vérifiez le
                            token / le scope <code>payments:read</code>).
                            Si <em>Allocations avec CLASS_ID</em> = 0, les rows existent mais n'ont pas de
                            <code>CLASS_ID</code> — impossible de les rattacher à un groupe.
                            Si <em>Classes avec START_DATE</em> ≪ <em>Classes récupérées</em>, la détection Début/Ajouts ne
                            fonctionne que sur une minorité de groupes.
                            Les libellés affichés ci-dessus sont ceux comparés à la regex <code>inscription</code> — si le
                            bon libellé n'apparaît jamais avec ce mot, la détection rate.
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="alert alert-light border mt-3 small">
                ...
                <ul class="mb-0 mt-1 ps-3">
                    <li><strong style="color:#6f42c1;">Début</strong> — étudiants dont le 1<sup>er</sup> mois payé est le
                        <em>même mois</em> que la date de démarrage du groupe (présents dès le lancement). Pour suivre le
                        cycle complet, choisissez une plage qui couvre le mois de démarrage du groupe.
                    </li>
                    <li><strong style="color:#28a745;">Ajouts</strong> — étudiants dont le 1<sup>er</sup> mois payé est
                        <em>après</em> le mois de démarrage du groupe (arrivés en cours de route).
                    </li>
                    <li><strong style="color:#dc3545;">Quittant</strong> — étudiants qui ont payé un mois mais n'ont pas
                        payé le mois suivant (1 mois manqué), et ne paient aucun autre groupe.</li>
                    <li><strong style="color:#fd7e14;">Changement</strong> — étudiants ayant payé 2 groupes différents avec
                        des paiements consécutifs séparés de ≤
                        {{ \App\Services\Crm\Stats\GroupEvolutionService::CHANGEMENT_WINDOW_DAYS }} jours. Le départ est
                        crédité au groupe qu'ils ont quitté.</li>
                    <li><strong style="color:#2196f3;">Actifs</strong> — compteur courant du CRM
                        (<code>CLASS_COUNT_STUDENTS_ACTIVE</code>), pas une inférence.</li>
                </ul>
            </div>
        @endif
    </div>
...
$chartGroups = array_map(
    fn($g) => [
        ...
        'actifs' => $g['actifs'],
        'debut_students' => $g['debut_students'] ?? [],
        'ajout_students' => $g['ajout_students'] ?? [],
        'quittant_students' => $g['quittant_students'] ?? [],
        'changement_students' => $g['changement_students'] ?? [],
...
@section('scripts')
    ...
    {{-- All groups data (with start/end dates for auto-fill) --}}
    <script type="application/json" id="crm-all-groups-data">{!! json_encode($allGroups ?? [], JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) !!}</script>
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="studentListModalLabel"></h5>
