...
@section('content')
    ...
    <div class="card mb-4">
        <div class="card-header d-flex align-items-center justify-content-between">
            <div>
                <h5 class="mb-0"><i class="ph-duotone ph-chart-line me-1"></i> Résumé des frais annuels</h5>
                <small class="text-muted">Vue d'ensemble des frais annuels</small>
            </div>
            <div class="d-flex align-items-center gap-2">
                <i class="ph-duotone ph-calendar-blank text-muted"></i>
                <span class="text-muted small">Année</span>
                <select id="annualSummaryYear" class="form-select form-select-sm" style="width:auto;">
                    @php $currentYear = (int) now()->format('Y'); @endphp
        ...
        <div class="card-body p-0">
            <div id="annualSummaryChart" style="height: 340px;"></div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ── Annual Summary Chart (5-series area) ─────────────────────────────────
    const annualData = @json($annualSummary);
    ...
    const fmtDH = v => new Intl.NumberFormat('fr-FR').format(v);

    const annualChartColors = {
        chiffre_affaire: '#dc2626',   // red
        collecte:        '#4680FF',   // blue
        reste_a_payer:   '#e58a00',   // orange
        depenses:        '#2ca87f',   // green
        encaissements:   '#e53e3e',   // deep red (small line)
    };
    ...
    function buildAnnualChart(data) {
        ...
        annualChart = new ApexCharts(document.querySelector('#annualSummaryChart'), {
            chart: {
                type: 'area',
                height: 340,
                toolbar: { show: false },
            ...
            series: [
                { name: "Chiffre d'affaire", data: data.map(m => m.chiffre_affaire) },
                { name: 'Collecté',          data: data.map(m => m.collecte) },
                { name: 'Reste à payer',     data: data.map(m => m.reste_a_payer) },
                { name: 'Dépenses',          data: data.map(m => m.depenses) },
                { name: 'Encaissements',     data: data.map(m => m.encaissements) },
            ],
            xaxis: {
                categories: data.map(m => m.month_label),
            ...
            colors: [
                annualChartColors.chiffre_affaire,
                annualChartColors.collecte,
                annualChartColors.reste_a_payer,
                annualChartColors.depenses,
                annualChartColors.encaissements,
            ],
            fill: {
                type: 'gradient',
                gradient: { opacityFrom: 0.35, opacityTo: 0.05, shadeIntensity: 1 },
            },
            stroke: { curve: 'smooth', width: [2, 2, 2, 2, 2] },
            ...
            crosshairs: {
                show: true,
                stroke: { dashArray: 4, color: '#adb5bd', width: 1 },
            },
            tooltip: {
                shared: true,
                intersect: false,
    ...
    buildAnnualChart(annualData);

    // Year selector — reload page with new year param
    document.getElementById('annualSummaryYear').addEventListener('change', function () {
        const url = new URL(window.location.href);
        url.searchParams.set('year', this.value);
        url.searchParams.delete('month_num'); // reset month when changing year
