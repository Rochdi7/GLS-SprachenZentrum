            ...
            <details class="gls-address-details" name="{{ $groupName }}">
                ...
                <div class="gls-address-body">
                    @foreach($branches as $index => $branch)
                        ...
                        @endphp
                        @if(!empty($branch['address']) && !empty($branch['maps_url']))
                            <a class="{{ $linkClass }}"
                        ...
                        @if(!empty($branch['phones']))
                            <div class="gls-address-phones">
                                @foreach($branch['phones'] as $phone)
                                    @php
                                        $digits = preg_replace('/\D/', '', $phone['n']);
                                        // Local 0XXXXXXXXX → 212XXXXXXXXX for wa.me
                                        $waDigits = str_starts_with($digits, '212') ? $digits : (str_starts_with($digits, '0') ? '212' . substr($digits, 1) : $digits);
                                        $type = $phone['t'] ?? 'call';
                                        $showCall = in_array($type, ['call', 'both'], true);
                                        $showWa   = in_array($type, ['whatsapp', 'both'], true);
                                    @endphp
                                    @if($showCall)
                                        <a class="gls-address-phone is-call" href="tel:+{{ $digits }}" aria-label="Appeler {{ $branchName }} : {{ $phone['n'] }}">
                                    ...
                                    @endif
                                    @if($showWa)
                                        <a class="gls-address-phone is-whatsapp" href="https://wa.me/{{ $waDigits }}" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp {{ $branchName }} : {{ $phone['n'] }}">
                                            <svg class="gls-address-phone-icon" viewBox="0 0 24 24" width="14" height="14" fill="currentColor" aria-hidden="true">
