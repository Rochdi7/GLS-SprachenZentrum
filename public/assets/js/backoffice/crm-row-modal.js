...
function crmRowModalInit() {
    ...
    const exportMatrixXlsx = () => {
        ...
        // CRITICAL: send the same SERVICE_LIST the modal received so the
        // export reproduces the exact same column set (including months
        // with no payments yet — Juin, Juillet, Août, etc).
        addInput('serviceList', JSON.stringify(lastMatrixData._serviceList || []));
        ...
        if (matrixBtn) {
            ...
            matrixBody.innerHTML = `
                <div class="crm-matrix-loader">
                    ...
                    <div class="crm-matrix-loader__sub" id="crmMatrixLoaderSub">
            ...
            const stages = [
                ...
                'Agrégation par étudiant et par service...',
                'Calcul des totaux et tri chronologique...',
                'Mise en forme de la matrice...',
            ];
            let stageIdx = 0;
            const subEl = document.getElementById('crmMatrixLoaderSub');
            const stageTimer = setInterval(() => {
                stageIdx = (stageIdx + 1) % stages.length;
                if (subEl) subEl.textContent = stages[stageIdx];
            }, 1600);
